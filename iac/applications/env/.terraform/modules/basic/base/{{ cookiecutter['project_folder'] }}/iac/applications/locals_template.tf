{% raw %}{% if app == "env" %}data "google_project" "project" {}
...
locals {
  ...
  ){% endif %}{% endraw %}{% if cookiecutter['include_iam'] == "Yes" %}
  # IAM
  ...
  {% raw %}{% if app == "env" %}service_accounts = merge(var.service_accounts, { "cloudbuild" : local.cloud_build_service_account }){% else %}service_accounts = merge(
  ...
  service_agents = {
    {% raw %}{% if app == "env" %}"aiplatform.googleapis.com" = {
  ...
  default_roles = {
    {% raw %}{% if app == "env" %}"aiplatform.googleapis.com" : ["roles/aiplatform.serviceAgent"],
    ...
    {% endif %}{% endraw %}"cloudbuild.googleapis.com" : ["roles/cloudbuild.serviceAgent"],{% raw %}{% if app == "env" %}
    "compute.googleapis.com" : ["roles/editor"],{% endif %}{% endraw %}
    "containerregistry.googleapis.com" : ["roles/containerregistry.ServiceAgent"],{% raw %}{% if app == "env" %}
    "ml.googleapis.com" : ["roles/ml.serviceAgent"],{% endif %}{% endraw %}
    "pubsub.googleapis.com" : ["roles/pubsub.serviceAgent"],{% raw %}{% if app == "env" %}
    "run.googleapis.com" : ["roles/run.serviceAgent"],{% endif %}{% endraw %}
  }
{% raw %}{% if app == "ops" %}
  ar_reader_roles = {
...
{% endif %}{% endraw %}
  project_iam = {
    "{{ cookiecutter['project_folder'] }}" = {
      ...
      "sa"         = merge(var.service_account_roles, local.default_roles{% raw %}{% if app == "ops" %}, local.ar_reader_roles{% endif %}{% endraw %})
